...
class CustomizedPOSController extends Controller
{

    public function CustomizedPOS(Request $request)
    {
        $user = $request->user()->userid;
        $customCourses = $request->input('chosenCoursesIds');
        ...
        // Extract IDs and sum credits
        $customCourseIds = collect($customCourses)->pluck('courseid')->toArray();
        $customCourseCredits = collect($customCourses)->pluck('credits')->sum();
        $completedCredits += $customCourseCredits;
        // Merge IDs with done courses
        $doneCourseIds = array_unique(array_merge($doneCourseIds, $customCourseIds));
        $remaining_credits = $total_credit_department - ($completedCredits + $customCourseCredits);
